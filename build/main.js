...
/*!**********************!*\
  !*** ./src/index.js ***!
  \**********************/
/*! exports provided: DATA_KEY, getData, addData, createInput, createList, updateList, createTodo */
...
function addData(storage, content) {
...
function updateList(storage, element) {
  element.innerHTML = getData(storage).map(item => `<li>${item}</li>`).join('');
}
function createTodo(storage) {
  createInput();
  createList();
}
